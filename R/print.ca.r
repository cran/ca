...
################################################################################
# 
# print.ca: (Generic) function for printing 'ca'-objects
#
#      Input: x   The 'ca' object which should be printed
#
#     Output:     'ca' object neatly formatted
# 
################################################################################
...
print.ca <- function(x, ...)
  ...
  obj <- x
  nd0 <- length(obj$sv)
#  nd2 <- dim(obj$rowcoord)[2]
#  if (nd > dim(obj$rowcoord)[2]) nd <- dim(obj$rowcoord)[2]
  nd  <- obj$nd
  if (is.na(nd)){
    nd <- 2
    } else {
